{ config, lib, ... }:
{
  programs.niri.settings = {
    binds = {
      ...
      "Mod+Y".action.spawn = ["rofi-rbw"];
      "Mod+O".action.spawn = ["cliphist-paste"];
      "Mod+Escape".action.spawn = ["bash" "-c" ''
        if [ "$(niri msg overview-state)" = "Overview is closed." ]; then
          ${lib.getExe config.programs.anyrun.package} &
        else
          ${lib.getExe config.programs.anyrun.package} close
        fi
      ...
      "Mod+Shift+1".action.spawn = [
        ...
        ''
          ...
          if [ $ACTIVE_WORKSPACE == 1 ]; then
            niri msg action focus-workspace-previous
            PREV_WORKSPACE=$(niri msg --json workspaces | jq '.[] | select(.is_active == true) | .idx')
            niri msg action focus-workspace-previous
            niri msg action move-column-to-workspace $PREV_WORKSPACE
          else
      ...
      "Mod+Comma".action = lib.mkForce {
        consume-window-into-column = {};
      };
      "Mod+Period".action.expel-window-from-column = {};
      "Mod+Slash".action.spawn = [ "dms" "ipc" "settings" "toggle" ];
